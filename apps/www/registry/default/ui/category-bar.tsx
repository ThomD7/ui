...
export default function CategoryBar({ children, thickness, percentage, cumulative, legend, className }: CategoryBarProps) {
    ...
    return (
        <div className={`flex flex-col items-center w-full ${className} ${percentage ? "mt-10" : ""}`}>
            {/* Progress Bar */}
            <div className={`${sizes.thickness} relative w-full bg-gray-300 rounded-full`}>
                    ...
                    {React.Children.map(children, (child, index) => {
                        ...
                        return (
                            <div
                                ...
                                className={`absolute top-0 left-0 h-full ${index === 0 ? "rounded-l-full" : ""} rounded-r-full hover:brightness-90`}
                                style={{
                    ...
                    <div className="absolute inset-0 pointer-events-none">
                        {markers.map((pos, index) => (
                            <div
                                ...
                                style={{
                                    left: `${cumulative ? pos : markers.slice(0, index + 1).reduce((a, b) => a + b, 0)}%`,
                ...
                <div className="flex gap-4 mt-2">
                    {React.Children.map(children, (child) => (
                        <div key={child.props.title} className="flex items-center gap-2">
                            <div className="w-4 h-4 rounded-full" style={{ backgroundColor: child.props.color }}></div>
